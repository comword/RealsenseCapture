...
public class StreamProfileSelector implements Comparable<StreamProfileSelector>, Parcelable {
    ...
    protected StreamProfileSelector(Parcel in) {
        mWidth = in.readInt();
        mHeight = in.readInt();
        mFps = in.readInt();
        mEnabled = in.readByte() != 0;
        mIndex = in.readInt();
        mProfiles = (List<StreamProfile>)in.readArrayList(StreamProfile.class.getClassLoader());
    }

    public static final Creator<StreamProfileSelector> CREATOR = new Creator<StreamProfileSelector>() {
        @Override
        public StreamProfileSelector createFromParcel(Parcel in) {
            return new StreamProfileSelector(in);
        }

        @Override
        public StreamProfileSelector[] newArray(int size) {
            return new StreamProfileSelector[size];
        }
    };
